...
impl Sudo {
    ...
    pub fn execute_opts<S: AsRef<OsStr>>(
        ...
        let runs_as_root = opts.user.is_none_or(is_root_user);
        if runs_as_root && !ctx.run_type().dry() {
...
/// What to do when a binary about to run under `sudo` resolves to a path a
/// non-root user could control.
///
...
/// still be swapped after this check (TOCTOU). The issue 2116 threat model
/// accepts that.
